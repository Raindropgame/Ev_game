...
public class breathTest : MonoBehaviour {
	...
	void Update () {
        ...
        if (CharacterAttribute.GetInstance().Breath < CharacterAttribute.GetInstance().MaxBreath && (CharacterControl.instance.currentState == state.normal || CharacterControl.instance.currentState == state.walk))
        {
